...
void Juego::actualizar() {
    ...
    // Asegurar que Goku no se quede atrás con el scroll acelerado
    if (goku->x() < 300 && velocidadScroll > 5) {
        goku->setX(goku->x() + velocidadScroll * 0.1); // Pequeño empujón
    }
}
...
void Juego::soltarGokuDeCuerda(Cuerda& cuerda) {
    ...
    // Usar el método setter en lugar de acceso directo
    goku->setVelocidadX(cuerda.largo * cuerda.velocidadAngular * cos(cuerda.angulo) * 0.7);

    goku->iniciarCaidaDesdeCuerda(extremo);
